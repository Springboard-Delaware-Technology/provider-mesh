...
| `db:verify`   | Confirms ledger hashes and applying role, the catalog checksum at the ledger head against the catalog now, the exact table, view, and function sets (each function's owner and `SECURITY DEFINER` flag), forced row-level security on protected tables, policies, triggers, role attributes, exact non-owner grants on tables and views, reference row counts, and connect grants (`expected-catalog.ts`) | Every check passed                     | —                                                                   | Any check failed                                                                                                     |
...
| `expected-catalog.ts`   | The maintained expected state per database; extended by every capability that adds a migration (C5 added the audit table, view, functions, triggers, and grants) |
