...
| Development ledger state | **Behind by one audit migration.** The C4 set was migrated 2026-09-20 by Judson Malone from the Replit workspace via `npm run db:rehearse`, `npm run db:migrate`, `npm run db:status`, `npm run db:verify` (rehearsal rolled back cleanly; status at head with zero pending and zero mismatches; `db:verify` 15 checks passed on each store), and the application started with `migrations=domain:1,audit:1`. C5 adds `audit/0002_audit_event.sql`, not yet applied in development: until the decision authority runs the same four commands from the workspace, `db:status` reports one pending audit migration, the startup assertion fails with `migration_ledger_mismatch`, and `db:verify` expects the audit table, view, and functions. After application, record the hash, time, and catalog checksum of the new row here |
...
| Audit ledger (`provider_mesh_audit`) | `0001_migration_ledger.sql` — sha256 `57243df4fc0a0cf8a054c49050b088a83452cc787aea0ec88ded378db23390ea`; applied 2026-09-20 14:06:02 UTC by `mesh_migrate`; catalog `e3dd18e560f0f1b00ec228611873e81a0de5d33706c898cca8241ce6e3977b7e`. `0002_audit_event.sql` — pending in development (see above); its file hash is the one `db:status` prints as pending |
...
| Audit store (C5, Foundation 001 §5.5) | `db/migrations/audit/0002_audit_event.sql`: `audit_event` (the `SEC-AUD-02` field groups with a paired absence reason per nullable column), the `SECURITY DEFINER` chain trigger owned by `mesh_migrate` (advisory lock; `sequence`, `prev_hash`, `hash`, `recorded_at` server-assigned; canonical serialization version 1 documented in the file), the append-only trigger, the `audit_chain_head` view, and grants leaving `mesh_audit_writer` with `INSERT` plus the head view and `mesh_audit_reader` with `SELECT`. The audit database must be UTF-8 (Neon and the CI container are). `npm run audit:verify` recomputes the chain as `mesh_audit_reader` through `PROVIDER_MESH_AUDIT_READER_URL`; `npm run audit:checkpoint` exports a checkpoint on demand as `mesh_audit_writer`; the application exports one after startup and hourly. Checkpoints go to the development `ObjectStore` adapter at `var/object-store` (git-ignored) under the same identity as the application — see §4 for what is not established |
...
| Audit chain | After `test:db`: `audit:verify` as `mesh_audit_reader`, then `audit:checkpoint` as `mesh_audit_writer` (checkpoint to the runner's `var/object-store`, discarded with the runner) and `audit:verify` again against that checkpoint (§5.5 rules 3–4; A14–A15) |
...
| Immutable object store and independent audit-checkpoint identity | Not established. Checkpoints are exported through the `ObjectStore` port to the development adapter (`var/object-store`, a local directory under the application's own identity), so they evidence the mechanism, not independence from the audit store's administrator (Foundation 001 §5.5 rule 4, §7.2) | Protected-continuity gate (ADR-001 §5.3; `SEC-AUD-04`) |
...
| 2026-09-20 | §1.8 audit store record added with Foundation 001 C5: the second audit migration and the development database's behind-by-one state with the human step that closes it; §2 CI audit-chain steps; §4 checkpoint store entry qualified | Implementation agent |
